...
{
  topic_property_ = new rviz_common::properties::RosTopicProperty("Image Overlay Topic", "image_overlays",
    rosidl_generator_traits::name<wolf_msgs::msg::ImageOverlayArray>(),
    "ImageOverlayArray topic to subscribe to.",
    this, SLOT(updateTopic()));

  tf_frame_property_ = new rviz_common::properties::TfFrameProperty("Fixed Frame", "map",
    "Fixed frame to use for positioning overlays.",
    this, nullptr, true);

  meters_per_pixel_property_ = new rviz_common::properties::FloatProperty("Meters per Pixel", 0.002,
...
void ImageOverlayDisplay::updateTopic()
{
  ...
  overlay_sub_ = raw_node_->create_subscription<wolf_msgs::msg::ImageOverlayArray>(
    topic,
    rclcpp::QoS(1),
    std::bind(&ImageOverlayDisplay::overlayCallback, this, std::placeholders::_1));
}

void ImageOverlayDisplay::overlayCallback(const wolf_msgs::msg::ImageOverlayArray::SharedPtr msg)
{
...
void ImageOverlayDisplay::processOverlay(const wolf_msgs::msg::ImageOverlay& overlay)
{
  OverlayData data;
  data.pose = overlay.pose;
  data.image = overlay.image;
  data.texture = std::make_shared<rviz_default_plugins::ROSImageTexture>();

  if (overlay.image.width == 0 || overlay.image.height == 0)
  {
    RCLCPP_WARN(raw_node_->get_logger(), "Received image with 0 width or height (ID %d), skipping.", overlay.id);
    return;
  }
  ...
  try
  {
    ...
    {
      RCLCPP_WARN(raw_node_->get_logger(), "Converted OpenCV image is empty or has 0 size (ID %d), skipping.", overlay.id);
      return;
    }
...
PLUGINLIB_EXPORT_CLASS(wolf_rviz_plugin::ImageOverlayDisplay, rviz_common::Display)
